<div align="center">
<!-- ⌨️ TYPING ANIMATION -->
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=24&pause=1000&color=00F7FF&center=true&vCenter=true&width=700&lines=Software+Developer;AI+Engineer;Game+Developer;Web+Creator;Problem+Solver"/>
<br>
<p align="center">
<img src="https://media.giphy.com/media/xT9IgzoKnwFNmISR8I/giphy.gif" width="600"/>
</p>
</div>

---

# 👨‍💻 About Me
```text
💻 Software Developer focused on Web, AI & Game Development
⚡ I build interactive applications and automation tools
🧠 Strong interest in algorithms, logic and clean code
🚀 Always learning and improving my skills
```

---

![snake](https://raw.githubusercontent.com/Nikushhaa/Nikushhaa/output/github-contribution-grid-snake.svg)

# 🛠️ Tech Stack
<div align="center">
<img src="https://skillicons.dev/icons?i=js,python,html,css,git,github,vscode,csharp"/>
</div>

---

# 🚀 Featured Projects
- 🤖 **Jarvis Assistant** → AI voice automation assistant  
- 🎮 **Reverse Game** → Reversi game with AI opponent  
- ⌨️ **CodePilot** →  Modern open-source developer toolbox
- 🌐 **Gaming Hub** → Modern gaming website UI  
- 🍽️ **Restaurant Design** → Responsive restaurant website  

---

<!-- GitHub README markdown doesn't support native lazy-loading (loading="lazy" is stripped on render),
     so stats/activity widgets below are grouped into a single block instead of separate sections —
     this cuts down on duplicate wrapper divs and repeated heavy widget calls. -->

# 📊 GitHub Stats & Activity
<div align="center">
<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Nikushhaa&show_icons=true&theme=tokyonight&hide_border=true"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Nikushhaa&layout=compact&theme=tokyonight&hide_border=true"/>
<br>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=Nikushhaa&theme=tokyonight&hide_border=true"/>
</div>

---

# 🏆 GitHub Trophies
<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=Nikushhaa&theme=tokyonight&no-frame=true&row=1&column=6"/>
</div>

---

# 📈 Focus Areas
- 🌐 Web Development  
- 🤖 Artificial Intelligence  
- 🎮 Game Development  
- ⚡ Automation Tools  
- 🧠 Algorithms & Problem Solving  

---

# 📫 Contact Me
<div align="center">
<a href="https://github.com/Nikushhaa">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>
</a>
<a href="https://www.instagram.com/nikushanakashidzee/">
<img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
</a>
<a href="https://discord.com/users/almightyduck2">
<img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white"/>
</a>
</div>

---

<!-- 🌊 FOOTER -->
<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=120&section=footer"/>
</div>
